...
async function handlePhotoMessage(message) {
  ...
  if (!caption.includes('\n')) {
    return sendMessage(chatId, '❌ Please include name, description, and at least one quantity option');
  }
...
async function handleConfirmOrder(chatId, username) {
  ...
  return sendMessage(chatId, '✅ Your order has been placed! You will be notified when it is confirmed.');
}
